...
## Ближайшие бизнес-инкременты

### 1. Естественный язык вместо технического формата

- принять фразы вроде «Поставь завтра в 19:00 созвон с Колей на полчаса»;
- учитывать часовой пояс пользователя, относительные даты и длительность;
- задавать короткий уточняющий вопрос при неоднозначности или нехватке данных;
- отделить `IntentModel` от бизнес-валидации: модель предлагает типизированный draft, но не выполняет действие;
- оставить детерминированную fake-модель для CI и добавить сменный OpenAI-compatible provider для локального
  или облачного LLM;
- проверять structured output общей схемой контрактов и не передавать в Action Service неполный payload.

Результат: Telegram принимает обычную пользовательскую фразу и показывает точную карточку встречи.

### 2. Настоящий календарь

- сохранить единый `calendar-mcp`, а провайдеры выбирать через стратегию `fake`/`google`;
- добавить Google OAuth connection flow и хранить refresh token только в зашифрованном виде;
- использовать тот же `calendar.create_event`, `requestKey` и защиту от дублей, что и у fake-провайдера;
...
### 3. Сквозная наблюдаемость после real N2N

- передавать W3C `traceparent` через Telegram Adapter, Gateway, Conversation, Agent, Action, Temporal и MCP;
- показывать полную последовательность span в Grafana Tempo;
- писать структурированные логи с `service.name`, `service.version`, `deployment.environment`, `trace_id`,
  `span_id` и безопасным `request_id`;
- добавлять в Loki метки `compose_service` и Kubernetes metadata через Collector, не задавая фиксированный
  `container_name`, который мешает нескольким replica;
- связать логи, trace, Temporal workflow ID и итоговый audit trail без записи prompt, токенов и персональных
  данных в обычные логи.

Результат: один реальный Telegram-запрос открывается в Grafana как последовательность всех сервисов, а логи
фильтруются по сервису, окружению и trace.
