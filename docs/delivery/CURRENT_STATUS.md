...
| `conversation-service` | Состояние диалога и прикладная оркестрация | Повтор сообщения не создаёт второе действие | Граница закреплена в ADR-0013, API выпущен в contracts `2.3.0`; репа ещё не создана | Создать TDD-каркас и хранение идемпотентного сообщения |
