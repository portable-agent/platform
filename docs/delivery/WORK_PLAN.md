...
Состояние: требует решения владельца продукта.

Перед кодом нужно вместе выбрать один сценарий. Предлагаемый первый кандидат — создание события
календаря с явным подтверждением. После выбора сначала пишется сквозной acceptance-тест и только затем
добавляются новые сервисы или интеграции.
